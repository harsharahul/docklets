...
#   TUNNEL_USER=                     # managed edges: your assigned user; the
#                                    # token then authenticates that user
#
...
# Pattern ranges like [!a-z0-9-] are locale-collated in bash 3.2, which lets
# uppercase slip through [a-z] under UTF-8 locales. Pin the C locale so the
# validation globs below mean ASCII, everywhere.
export LC_ALL=C
...
# TUNNEL_USER=               # managed edges: your assigned user
EOF
...
# Managed multi-tenant edges assign a user; the token then authenticates
# that user instead of the whole edge. Optional: unset means classic
# edge-wide token auth.
TUNNEL_USER="${TUNNEL_USER:-}"
case "$TUNNEL_USER" in
  *[!a-z0-9-]*|-*) echo "TUNNEL_USER must be lowercase kebab-case" >&2; exit 1 ;;
esac
...
if [ -n "$TUNNEL_USER" ]; then
  AUTH_LINES="user = \"$TUNNEL_USER\"
metadatas.token = \"$TUNNEL_TOKEN\""
else
  AUTH_LINES="auth.method = \"token\"
